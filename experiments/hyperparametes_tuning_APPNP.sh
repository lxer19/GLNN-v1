...
aggregated_result_file="exp_results/APPNP_MLP_pubmed_tran.txt"
printf "Student\n" >> $aggregated_result_file

for rate in $(seq 0.001 0.0005 0.015)
do
    for decay in $(seq 0 0.0005 0.001)
    do
        for dropout in $(seq 0 0.2 0.8)
        do
            printf "%10s\t" $rate >>"%10s\t" $decay >>"%10s\t" $dropout >> $aggregated_result_file
            python train_student.py --exp_setting "tran" --teacher "APPNP" --dataset 'pubmed' --learning_rate $rate --weight_decay $decay --dropout_ratio $dropout\
